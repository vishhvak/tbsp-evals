...
if __name__ == "__main__":
    ...
    test_samples = df.sample(args.sample_number, random_state=1).reset_index(drop=True)
    test_samples["input"] = test_samples.apply(lambda x: create_chat_prompt(x['description']), axis=1)

    client = OpenAI()

    test_samples['gpt-response'] = test_samples.input.progress_apply(get_answer)
    ...
    test_samples['correct'], test_samples['reason'] = zip(*test_samples.apply(lambda row: compare_jsons(row['answer'], row['gpt-response']), axis=1))
    ...
    test_samples['nutrients'] = test_samples['answer'].apply(lambda x: {k: v for k, v in sorted(json.loads(x).items(), key=lambda item: item[0])})
    test_samples['estimated_nutrients'] = test_samples['gpt-response'].apply(lambda x: {k: v for k, v in sorted(json.loads(x).items(), key=lambda item: item[0])})
    keys_to_export = ['item_description', 'nutrients', 'estimated_nutrients', 'correct', 'reason']
